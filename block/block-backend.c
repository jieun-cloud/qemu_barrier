                                ...
                                BdrvRequestFlags flags)
{
    ...
    ret = bdrv_co_pwritev(blk->root, offset, bytes, qiov, flags);
		bs->active_write_req = true;	/* kms-modify : Disable barrier while processing write request */
    bdrv_dec_in_flight(bs);
    return ret;
}

/* kms-modify : Barrier enabled */
int coroutine_fn blk_co_barrier_pwritev(BlockBackend *blk, int64_t offset,
                                unsigned int bytes, QEMUIOVector *qiov,
                                BdrvRequestFlags flags)
{
    int ret;
    ...
    ret = bdrv_co_barrier_pwritev(blk->root, offset, bytes, qiov, flags);	/* kms-modify : Barrier enabled */
    bdrv_dec_in_flight(bs);
    return ret;
}
...
/* kms-modify : Barrier enabled */
static void blk_aio_barrier_write_entry(void *opaque)
{
    BlkAioEmAIOCB *acb = opaque;
    BlkRwCo *rwco = &acb->rwco;
    QEMUIOVector *qiov = rwco->iobuf;
    ...
    assert(!qiov || qiov->size == acb->bytes);
    rwco->ret = blk_co_barrier_pwritev(rwco->blk, rwco->offset, acb->bytes,
                               qiov, rwco->flags);	/* kms-modify : Barrier enabled */
    blk_aio_complete(acb);
}
...
/* kms-modify : Barrier enabled */
BlockAIOCB *blk_aio_barrier_pwritev(BlockBackend *blk, int64_t offset,
                            QEMUIOVector *qiov, BdrvRequestFlags flags,
                            BlockCompletionFunc *cb, void *opaque)
{
    return blk_aio_prwv(blk, offset, qiov->size, qiov,
...
/* kms-modify : Barrier enabled */
static void blk_aio_barrier_flush_entry(void *opaque)
{
    BlkAioEmAIOCB *acb = opaque;
    BlkRwCo *rwco = &acb->rwco;

    rwco->ret = blk_co_barrier_flush(rwco->blk); /* kms-modify */
    blk_aio_complete(acb);
}
...
/* kms-modify : Barrier enabled */
BlockAIOCB *blk_aio_barrier_flush(BlockBackend *blk,
                          BlockCompletionFunc *cb, void *opaque)
{
    return blk_aio_prwv(blk, 0, 0, NULL, blk_aio_barrier_flush_entry, 0, cb, opaque); /* kms-modify */
}
...
/* kms-modify : Barrier enabled */
int blk_co_barrier_flush(BlockBackend *blk)
{
    if (!blk_is_available(blk)) {
        return -ENOMEDIUM;
    }

    return bdrv_co_barrier_flush(blk_bs(blk));
    //return bdrv_co_barrier(blk_bs(blk));
}
